    ...
    <div className='max-w-5xl mx-auto py-10 px-4 font-permanentMarker text-center bg-primary-claro rounded-lg my-20'>
      ...
      <div className='flex justify-between mt-4 max-w-3xl mx-auto px-4 text-centerfont-bold'>
        ...
        <div className='flex grid grid-cols-1'>
          <h4 className="w-20 text-sm sm:text-lg mx-2">PRECIO</h4>
        </div>
        ...
        {platos.map((plato) => (
          <div key={plato.id} className=" p-4 rounded flex justify-between">
            <div className='text-left'>
              ...
              <p className="text-sm text-white-600 w-45 sm:w-83">{plato.descripcion}</p>
            </div>
            <div className="mt-2 flex font-bold grid grid-cols-1 text-center">
              {plato.precio && (
                <p className="text-sm w-20 mx-2">${(plato.precio)}</p>
              )}
            </div>
          </div>
        ))}
